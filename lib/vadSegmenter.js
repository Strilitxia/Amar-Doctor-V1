...
// Whisper speech-to-text — the default STT engine, and the only one that
// keeps audio inside this app's own backend. (lib/webSpeechRecognizer.js is
// an opt-in alternative behind a toggle; it does its own endpointing and
// does not use this segmenter.)
//
...
async function createSileroSegmenter({ stream, onSpeechStart, onSpeechEnd, onError }) {
  ...
  const vad = await MicVAD.new({
    ...
    // These were all tuned tighter than @ricky0123/vad-web's own defaults
    // (0.3 / 0.25 / 800ms pad / 1400ms redemption), which cost onset audio:
    // a higher positive threshold means Silero only concedes "this is
    // speech" a few frames later, and only preSpeechPadMs of audio before
    // that point is kept. At 300ms of pad that left very little room on a
    // soft word-initial sound. Losing under ~300ms of onset turned out not
    // to change the transcript, but losing ~500ms dropped the first word
    // outright, so the pad is back to the library default — it costs
    // nothing but a little extra audio per utterance.
    positiveSpeechThreshold: 0.5,
    negativeSpeechThreshold: 0.35,
    minSpeechMs: 250, // ~drops coughs/clicks (option is *Ms, not *Frames)
    // Trailing silence before the turn is considered over. vad-web keeps
    // these redemption frames in the emitted segment, so this is not a
    // tail-clipping risk — it is purely the turn-end latency knob. Raise it
    // if mid-sentence pauses are splitting one utterance into two, which
    // hurts accuracy badly (Whisper loses the sentence context).
    redemptionMs: 900,
    preSpeechPadMs: 800,
...
function createEnergySegmenter({ stream, onSpeechStart, onSpeechEnd, onError }) {
  ...
  const SILENCE_END_MS = 900; // keep in step with redemptionMs on the Silero path
